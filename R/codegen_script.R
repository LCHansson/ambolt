  ...
  for (mod_out in module_outputs) {
    ...
    if (mod_out[["type"]] == "table") {
      ...
      if (!is.null(card_def[["fields"]]) && length(card_def[["fields"]]) > 0) {
        ...
        # R's `$` does partial matching on lists by default, so the older
        # `f$label` would silently match `f$labels` (a multi-element vector)
        # and corrupt `parts` to length-N. Same risk: `f$badge` ↔ `f$badge_color_key`.
        # See `tests/testthat/test-codegen-partial-match.R` for the audit
        # that guards against regressions across all codegen branches.
  ...
  state_lines <- if (length(inputs) > 0) {
    vapply(inputs, function(i) {
      ...
      if (i[["type"]] %in% c("search_results_panel", "multi_view_panel")) {
        ...
        # Two state variables: _start and _end. Matches codegen_markup.R's
        # `bind:start={id_start} bind:end={id_end}` binding. Accepts a
        # length-2 vector via `args$value` (or args$start / args$end);
        # falls back to today() for both.
        v <- i[["args"]][["value"]]
        today <- format(Sys.Date(), "%Y-%m-%d")
        start_val <- if (!is.null(v) && length(v) >= 1) v[[1]] else (i[["args"]][["start"]] %||% today)
        end_val   <- if (!is.null(v) && length(v) >= 2) v[[2]] else (i[["args"]][["end"]]   %||% today)
        paste0(sprintf("  let %s_start = $state('%s');", i[["id"]], start_val), "\n",
               sprintf("  let %s_end = $state('%s');",   i[["id"]], end_val))
